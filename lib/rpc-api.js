...
(function() {
	...
	exports.createClient = function(rpcclient) {
		// Add methods
		for (var k in methods)
			addMethod(rpcclient, k, methods[k]);

		return rpcclient;
	};

	function addMethod(rpcclient, k, method) {
		switch (method.type) {
			case 'readable': rpcclient[k] = createStreamMethodWrapper(rpcclient, k, 'createReadStream'); break;
			case 'writable': rpcclient[k] = createStreamMethodWrapper(rpcclient, k, 'createWriteStream'); break;
			case 'duplex':   rpcclient[k] = createStreamMethodWrapper(rpcclient, k, 'createStream'); break;
	...
	function createStreamMethodWrapper(rpcclient, methodname, streamMakeFn) {
		return function() {
			// Send RPC methodname and args as the substream meta
			var args = Array.prototype.slice.call(arguments);
			args.unshift(methodname);
			debug.logMX('client, sending streamcall', args);
			return rpcclient._mx[streamMakeFn](args);
		};
	}
})();