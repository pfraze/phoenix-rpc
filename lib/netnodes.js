...
exports._setup = function(opts) {
  ...
  this.netNodesDB.get('sync-state', function(err, state) {
    this.netState = state || { lastSync: null }
  }.bind(this))
}
...
exports.syncNetwork = function(opts, cb) {
  ...
  if (opts.ifOlderThan && (+(new Date()) - this.netState.lastSync) < opts.ifOlderThan)
    return cb(null, {})

  // Update net state
  this.netState.lastSync = +(new Date())
  this.netNodesDB.put('sync-state', this.netState)
